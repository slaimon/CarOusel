...
class LampGroup {
   public:
      ...
      void updateLightMatrixUniform(unsigned int i, shader s, const char* uniform_name) {
         lampProjectors[i].updateLightMatrixUniform(s, uniform_name);
      }

      void bindFramebuffer(unsigned int i) {
         lampProjectors[i].bindFramebuffer();
      }

      void bindTexture(unsigned int i) {
         lampProjectors[i].bindTexture(lampTextureSlots[i]);
      }

      glm::mat4 lightMatrix(unsigned int i) {
         return lampProjectors[i].lightMatrix();
      }
